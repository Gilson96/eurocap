...
const Menu = () => {
    ...
    return (
        <nav className='flex fixed top-0 w-full z-[99999]' >
            <div className='flex bg-white items-center justify-between w-full h-20 border border-slate-300 shadow-md p-3' id='navigator'>
                <Bars3Icon onClick={(() => { setMenuDropDown(!menuDropDown) })} className='w-12 h-12 text-red-500 text-left mr-2 mt-1' />
                {menuDropDown && <MenuDropDown />}
                <Link to='/' className='list-none no-underline text-black' ><h1 className='text-2xl text-left w-full mt-2 md:text-red-500' id='ecocap'>ECOCAP INVESTEMENTS</h1></Link>
